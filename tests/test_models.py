...
def test_passenger_direction_for_up_and_down_trips() -> None:
    ...
    assert Passenger(id=1, request_time=0, start_floor=1, destination_floor=4).direction == Direction.UP
    assert Passenger(id=2, request_time=0, start_floor=4, destination_floor=1).direction == Direction.DOWN
...
def test_passenger_rejects_same_origin_and_destination() -> None:
    ...
    with pytest.raises(ValidationError, match="start and destination floors"):
...
def test_models_validate_building_specific_bounds() -> None:
    ...
    passenger = Passenger(id=1, request_time=0, start_floor=1, destination_floor=4)
    elevator = Elevator(id=1, current_floor=4, capacity=2)
